...
**A sınıfı — güncel kaynak (2026-08-27):** `1224 passed, 6 skipped, 345 subtests` temiz build kapısında yeniden ölçülecek. Güncel kaynak `e46139322755469362b764fcb35e81aaa21802b5`; R7 açılış bildirimi düzeltmesine iki durumlu Güvenli Depo test ekranı ve dış PATH DLL kontaminasyonunu engelleyen paketleme regresyonu eklendi. Preview kataloğu 44 yüzey/188 durumdur; smoke ve geometri bulgusu 0, gerçek credential erişimi 0.

**A→B/C boşluğu:** Yayımlanmış v4.3 artifact `446fc78` kaynaklıdır ve R7 değişikliklerini içermez. Ara `9c7c46b` ile Codex runtime DLL'leri bulaşmış `24e1521` buildleri proje dışındaki geçici kanıt klasörlerine alındı; güncel kaynak için kanıt sayılmaz. Kontrollü PATH ile yeni temiz build sonrasında fail-keyring izolasyonunda splash sırasında modal olmadığını, ana pencere görünürken uyarının yalnız bir kez çıktığını ve normal kapanışı doğrulayan frozen UI smoke gerekir. Bu yapılmadan `e461393` için B veya C kanıtı iddia edilmez.
