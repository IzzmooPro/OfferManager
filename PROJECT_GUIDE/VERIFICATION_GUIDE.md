...
**A sınıfı — güncel kaynak (2026-08-23):** `1225 passed, 4 skipped, 345 subtests`. Ölçüm R7 açılış bildirimi değişikliğini içeren `1458e7e572363564bc02bdd54e8b25026fd5b89e` üzerinde `python -m pytest tests -q` ile alındı. Hedefli R7 paketi ayrıca `66 passed, 2 subtests` verdi; `py_compile` ve `git diff --check` temizdi.

**A→B/C boşluğu:** Mevcut v4.3 artifact `446fc78` kaynaklıdır ve yeni R7 açılış bildirimi davranışını içermez. Yeni temiz build sonrasında fail-keyring izolasyonunda splash sırasında modal olmadığını, ana pencere görünürken uyarının yalnız bir kez çıktığını ve normal kapanışı doğrulayan frozen UI smoke gerekir. Bu yapılmadan `1458e7e` için B veya C kanıtı iddia edilmez.
