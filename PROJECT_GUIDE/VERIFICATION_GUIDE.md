...
**A sınıfı — güncel kaynak (2026-08-27):** exact `0d9b6c2d7fbfd469eff63b0ed34a01c6097b0a8f` temiz build kapısında `1226 passed, 5 skipped, 346 subtests`. Installer upgrade temizleme düzeltmesi ve kırmızı→yeşil regresyon testi bu artifact'a build edilmiştir. R7 açılış bildirimi, iki durumlu Güvenli Depo test ekranı ve dış PATH/tırnaklı Python komutu regresyonları korunur. Önceki `8fe66bd` artifact'ının build anı sonucu `1224 passed, 7 skipped, 346 subtests` olarak tarihsel kayıttır.

**v4.4 A→B/C durumu — GEÇTİ:** Exact `36741DD2…52CFCE` v4.4 EXE için değişiklik-etkili B geçti. Null-keyring: modal 0, ikinci örnek exit 0 ve ilk süreç yaşadı. Fail-keyring: tek Güvenli Depo modalı, ana pencere native disabled, modal enabled. İki tur WM_CLOSE, tek kapanış yedeği, DB close, exit 0; iki DB `integrity_check=ok`, FK 0; forbidden marker/süreç sızıntısı 0. Kapsam B-1/B-2/B-3/B-5'tir; B-4/B-6/B-7/B-8/B-9 tekrarlanmadı ve tarihsel kanıtları exact v4.4'e devredilmedi.

**C sınıfı — v4.4 için TAM GEÇTİ:** Exact `TeklifYonetim_Setup_v4.4.exe` (`50.886.113 B / C8BEAE23…6C681`) ile mevcut eski v4.4 kurulumu → düzeltilmiş exact v4.4 yerinde upgrade, ilk kurulu smoke, uninstall, temiz reinstall ve ikinci kurulu smoke tamamlandı. Üç yükseltilmiş işlem exit 0 ve restart yok; upgrade ve reinstall ağaçları dist ile `256/256`, fark `0/0/0`; önce kalan 39 `api-ms-win-*` DLL ve `ucrtbase.dll` artık yok. Registry/AppId/dört kısayol doğru; kaldırmada üç installer kısayolu silindi, özel AIO kısayolu korundu. Snapshot anında kurulum `298/298`, veri `30/30`, yedek `16/16` rollback hash farkı `0/0/0`; ana DB `312C189C…9D0CE` bütün zincirde korundu, `integrity_check=ok`, FK 0. Salt-okunur probun oluşturduğu boş WAL ve 32.768 B SHM installer değişikliği sayılmadı; sonraki veri `32/32`, yedek `16/16` hashleri korundu. Upgrade ve reinstall sonrası toplam dört null/fail-keyring kurulu smoke normal kapandı; gerçek Credential Manager ve Computer Use kullanılmadı. Üç UAC onayı kullanıcı tarafından elle verildi. Bu C kanıtı tag/Release veya canlı updater D1/D2 kanıtı değildir.

**Release süreç kapısı — KIRMIZI:** Normal ve `--artifacts` denetimleri temizdir; `BUILD_AND_PACKAGING.md` ile `RELEASE_CHECKLIST.md` artık `0d9b6c2`'ye göre günceldir. `--release` yine exit `1` ile üç hata raporlar: bu iki belgenin build sonrası izin dışı çalışma ağacı değişikliği olması ve dürüst sonuç `release_candidate_ready=false`. Artifact'ın A/B/C sonucu bundan etkilenmez. Beş kanıt belgesi commit edilip o yeni exact committen temiz build alınmadan tag/Release yapılmaz.
