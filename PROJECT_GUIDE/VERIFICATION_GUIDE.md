...
**A sınıfı — güncel kaynak + exact kanıt commit'i + hedefli B + tam C (2026-09-05):** İşlevsel kaynak `efcfdcb`; exact `649575efd0b905ce84e3a055bc958aa4d32d30eb` girdisinde `packaging/Kurulum-Yap.bat --no-pause` exit `0`; build kapısı `1242 passed, 6 skipped, 360 subtests`. EXE `9.725.826 B / 3292F3CBAC7D…5E37F / 4.5.0.0 / v4.5`, dist `256 dosya / 168.681.030 B`; installer `50.882.931 B / 400AD27F58E1…AB21AF / 4.5.0.0`. Hedefli Frozen B `22/22`, iki izole null/fail-keyring turunda B-1/B-2/B-3/B-5 için geçti; B-4/B-6/B-7/B-8/B-9 tekrarlanmadı. Tam C'de önceki kurulu v4.5 → exact `649575e` v4.5 upgrade, iki profilli kurulu smoke, uninstall, temiz reinstall ve ikinci iki profilli kurulu smoke geçti; kurulu smoke toplam `44/44`. Upgrade/reinstall ağaçları dist ile `256/256`, fark `0/0/0`; uninstall program/registry/üç yönetilen kısayolu kaldırdı ve özel AIO kısayolunu korudu; reinstall registry `v4.5`, kısayollar ve exact EXE'yi geri getirdi. Gerçek veri `31/31`, fark `0/0/0`; ana DB `312C189C…9D0CE`, bütünlük temiz. Frozen kanıt `14136926…34415C`, birleşik C kanıtı `B0598914…1D8DA`. Bu paragraf yerel A/B/C kapsamıdır; public yayın ve D1/D2 aşağıdaki ayrı sonuçla doğrulandı.

**v4.5 release ve canlı updater — GEÇTİ (2026-09-05):** Anotasyonlu `v4.5` tag peeled hedefi exact build commit `649575e`; public/latest Release tek `TeklifYonetim_Setup_v4.5.exe` asset'iyle yayımlandı. Read-back ve bağımsız indirme `50.882.931 B / 400AD27F…AB21AF` olarak yerel installer ile byte-eşit. D1 kaynak güven zinciri `12/12` geçti. D2'de exact public v4.4 istemci canlı v4.5'i gördü, indirdi ve kurdu; final EXE `3292F3CB…5E37F`, registry `v4.5`, kurulum ağacı `256/256` fark `0/0/0`. Gerçek veri `31/31` fark `0/0/0`, DB `312C189C…9D0CE`, bütünlük temiz; final görünür smoke DB close, exit 0 ve süreç sızıntısı 0 ile kapandı. D2 kanıtı `F5106F58…3F343`.
...
**C sınıfı — v4.4 için TAM GEÇTİ:** Exact `TeklifYonetim_Setup_v4.4.exe` (`50.889.592 B / 4D074FB0…B5FC1`) ile mevcut v4.4 → exact `35f23ac` v4.4 yerinde upgrade, ilk kurulu smoke, uninstall, temiz reinstall ve ikinci kurulu smoke tamamlandı. Üç yükseltilmiş işlem exit 0 ve restart yok; upgrade ve reinstall ağaçları dist ile `256/256`, fark `0/0/0`; eski runtime DLL kalıntısı yok. Registry/AppId/dört kısayol doğru; kaldırmada üç installer kısayolu silindi, özel AIO kısayolu korundu. Snapshot anında kurulum `258/258`, veri `32/32`, yedek `16/16` rollback hash farkı `0/0/0`; ana DB bütün zincirde korundu, `integrity_check=ok`, FK 0. Upgrade ve reinstall sonrası iki ayrı null/fail-keyring kurulu smoke normal kapandı; gerçek Credential Manager ve Computer Use kullanılmadı. Bu C kanıtı tag/Release veya canlı updater D1/D2 kanıtı değildir.

**Release ve canlı updater — GEÇTİ:** Exact `35f23ac` build sonrasında yalnız izinli kanıt yolları güncellendi. Anotasyonlu `v4.4` tag exact build commit’ini gösterir; public/latest GitHub Release tek `TeklifYonetim_Setup_v4.4.exe` asset’iyle yayımlandı. Read-back ve bağımsız indirme `50.889.592 B / 4D074FB0…B5FC1` olarak yerel installer ile byte-eşit. D1 kaynak güven zinciri ve exact paketli public v4.3 → public v4.4 gerçek D2 teslimatı geçti; final kurulu EXE `ACABD2DE…53FF8 / v4.4`, ana DB hash’i korundu, normal kapanış exit 0 ve süreç sızıntısı 0.
