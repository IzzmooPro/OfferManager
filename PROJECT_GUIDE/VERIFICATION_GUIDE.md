...
**A sınıfı — güncel kaynak (2026-08-27):** `1223 passed, 6 skipped, 345 subtests`. Güncel kaynak `c6c7cdf35faef465f85120a82e5862c2bfa15f55`; R7 açılış bildirimi düzeltmesine iki durumlu Güvenli Depo test ekranı eklendi. Preview kataloğu 44 yüzey/188 durumdur; smoke ve geometri bulgusu 0, gerçek credential erişimi 0. Üç ek skip proje kökünde yeni artifact bulunmayan ara durumdandır.

**A→B/C boşluğu:** Yayımlanmış v4.3 artifact `446fc78` kaynaklıdır ve R7 değişikliklerini içermez. Ara `9c7c46b` build proje dışındaki geçici kanıt klasörüne alındı; güncel kaynak için kanıt sayılmaz. Yeni temiz build sonrasında fail-keyring izolasyonunda splash sırasında modal olmadığını, ana pencere görünürken uyarının yalnız bir kez çıktığını ve normal kapanışı doğrulayan frozen UI smoke gerekir. Bu yapılmadan `c6c7cdf` için B veya C kanıtı iddia edilmez.
