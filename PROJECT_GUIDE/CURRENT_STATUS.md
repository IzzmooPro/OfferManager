...
- **v4.3 artifact/yayın durumu:** Public artifact ve yayın kanıtları `446fc78` için geçerlidir. Güncel kaynak `e461393` R7 runtime düzeltmesini, Güvenli Depo test ekranını ve PATH kontaminasyonu regresyonunu içerdiği için `artifact_verification_status = stale_source_changed`, `release_candidate_ready = false`. Ara buildler proje dışındaki geçici kanıt klasörlerine alındı; public/latest release değiştirilmedi.
...
- **Güncel kaynak: `e46139322755469362b764fcb35e81aaa21802b5`; yayımlanmış v4.3 artifact build girdisi: `446fc780131dd77a8a4dcf630f8baa8287b367dd`.** R7 runtime düzeltmesi, yeni 44. yüzey olan Güvenli Depo test ekranı ve build PATH kontaminasyonu regresyonu yayımlanmış pakette yoktur.
- Güncel kaynak kapısı: **1224 passed, 6 skipped, 345 subtests** olarak temiz build kapısında yeniden ölçülecek; yeni build ve frozen smoke beklenir.
...
- `source_commit = e46139322755469362b764fcb35e81aaa21802b5`; `built_from_commit = 446fc780131dd77a8a4dcf630f8baa8287b367dd`. Yayımlanmış artifact R7 değişikliklerini içermez; proje kökündeki build çıktıları yeni temiz build öncesi boştur. Ara ve PATH-kontamine buildler yalnız geçici kanıt klasörlerindedir.
...
- `446fc78` public artifact'ı için B/C/release/D1/D2 kanıtı geçerlidir. Güncel `e461393` kaynak için `release_candidate_ready=false`; yeni temiz build ve R7 frozen UI smoke beklenir.
